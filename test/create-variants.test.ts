...
describe('createVariants', () => {
  ...
  describe('boundary conditions', () => {
    ...
    test('should handle config with many variants', () => {
      const config = {
        base: 'component',
        variant: {
          default: 'var1', v2: 'var2', v3: 'var3', v4: 'var4', v5: 'var5'
        },
        size: {
          default: 'size1', s2: 'size2', s3: 'size3'
        }
      };
  ...
  describe('range and edge cases', () => {
    ...
    test('should handle very long class names', () => {
      const longName = 'very-long-class-name-that-goes-on-and-on-with-many-hyphens-and-descriptive-words';
      const config = {
        base: longName,
  ...
  describe('config structure variations', () => {
    ...
    test('should work with special character keys', () => {
      ...
      const variants = createVariants(config);
      expect(variants({ variant: 'with-dash' })).toBe('base var-dash size-default');
      expect(variants({ variant: 'with_underscore' })).toBe('base var-underscore size-default');
      expect(variants({ variant: 'with.dot' })).toBe('base var-dot size-default');
    });
  });
    ...
    test('should maintain consistent mapping between keys and values', () => {
      ...
      variantKeys.forEach(vKey => {
        sizeKeys.forEach(sKey => {
          ...
          const expectedVariant = vKey === 'default' ? basicConfig.variant.default : basicConfig.variant[vKey];
          const expectedSize = sKey === 'default' ? basicConfig.size.default : basicConfig.size[sKey];
          const expected = ['btn', expectedVariant, expectedSize].filter(Boolean).join(' ');
