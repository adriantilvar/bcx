...
describe("cx", () => {
  ...
  describe("Input format conformance", () => {
    ...
    test("should handle strings with special characters", () => {
      expect(cx("foo-bar", "baz_qux", "test@example")).toBe(
        "foo-bar baz_qux test@example"
      );
    });

    test("should handle objects with special characters in keys", () => {
      expect(
        cx({ "foo-bar": true, baz_qux: true, "test@example": false })
      ).toBe("foo-bar baz_qux");
    });
  ...
  describe("Output validation", () => {
    ...
    test("should contain only valid class names and spaces", () => {
      ...
      expect(result).toMatch(/^[\w\-_@🎉café naïve ]*$/u);
    });
  });
});
